...
func TestProbeAndUpdateRepoPushAccessUsesGitHubViewerPermissionForReadWriteWithoutDryRunFallback(t *testing.T) {
	t.Parallel()
	...
	if err := a.Git.AddOrigin(repoPath, "https://niieani.github.com/acme/demo.git"); err != nil {
		t.Fatalf("add origin: %v", err)
	}
	if err := os.WriteFile(filepath.Join(repoPath, "README.md"), []byte("hello\n"), 0o644); err != nil {
		t.Fatalf("write readme: %v", err)
	...
	a.RunCommand = func(name string, args ...string) (string, error) {
		...
		return `{"viewerPermission":"WRITE"}`, nil
	}

	meta := domain.RepoMetadataFile{
		RepoKey:             "software/demo",
		Name:                "demo",
		OriginURL:           "https://niieani.github.com/acme/demo.git",
		PushAccess:          domain.PushAccessUnknown,
		BranchFollowEnabled: true,
	}
	...
	if updated.PushAccess != domain.PushAccessReadWrite {
		t.Fatalf("push access=%q, want %q", updated.PushAccess, domain.PushAccessReadWrite)
	}
	if strings.TrimSpace(updated.PushAccessCheckedRemote) != "origin" {
		t.Fatalf("checked_remote=%q, want origin", updated.PushAccessCheckedRemote)
	}
