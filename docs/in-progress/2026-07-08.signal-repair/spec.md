...
4. **Notification stream is a single aggregated message per sync run**, deduped by a
   ...
   whole-set deduplication is always on.
...
- every repo with `state == blocked`, unless in quiet period
  (`now - last_activity_at < attention.quiet_hours`, default 2h — you were just in that
  repo; you know)
- every repo with `state == wip` whose `last_activity_at` is older than
  `attention.wip_stale_hours` (default 24h)
- `pending` repos are never eligible (visible in status/overview only)

Go exports the eligible fleet snapshot, deterministic fingerprint, and
`attention.throttle_minutes`. The native app owns permission, persistence, digest
delivery, and click routing.
...
Config wizard: expose the attention thresholds and throttle. `ConfigFile.Version`
stays 1; the removed section is rejected explicitly with no alias or migration.
...
4. **Remote-format warning + verified autofix**: scan writes warning not state; sync
   ...
   present.
5. **Attention contract**: attention set (blocked minus quiet-period, stale wip),
